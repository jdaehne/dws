...
//            }
    ...
    public function parse ($tpl, $phs) {
        ...
        if (strpos(trim($tpl), '@CHUNK ') === 0) {
            ...
            // Support basic placeholders iN @CHUNK tags #593
            foreach ($phs as $key => $value) {
                if (is_scalar($value)) {
                    $chunkName = str_replace('[[+' . $key . ']]', $value, $chunkName);
                }
            }
...
. '</script>';
    ...
    {
        if ($this->normalParser instanceof modParser) {
