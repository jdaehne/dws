        ...
        if (in_array($fileExtension, $imageExtensions, true)) {
            ...
            if ($imagine) {
                try {
                    ...
                    $original = $imagine->load($image['content']);

                    // Use the autorotate filter to rotate the image if needed
                    $filter = new Autorotate();
                    $filter->apply($original);

                    // Avoid black backgrounds on images with transparency
                    $palette = new RGB();
                    $img = $imagine->create($original->getSize(), $palette->color('ffffff'));
                    $img->paste($original, new Point(0,0));
                    ...
                    $binary = $img->get($fileExtension);
                    $this->source->updateObject($this->path . $_FILES['file']['name'], $binary);
                } catch (Exception $e) {
                    $this->modx->log(modX::LOG_LEVEL_ERROR, 'Exception ' . get_class($e) . ' while processing image upload ' . $url . '  for crop into imagine: [' . get_class($e)  . '] ' . $e->getMessage());
