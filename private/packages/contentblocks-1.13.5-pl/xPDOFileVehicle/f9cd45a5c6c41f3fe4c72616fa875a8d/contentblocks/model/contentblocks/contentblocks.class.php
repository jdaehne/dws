...
//            }
        ...
        foreach ($this->modx->getIterator('cbField') as $field) {
            ...
            if (array_key_exists($input, $this->inputs) && $this->inputs[$input] instanceof cbBaseInput) {
                ...
                foreach ($topics as $topic) {
                    if ($this->modx->controller instanceof modManagerController || $this->modx->controller instanceof \MODX\Revolution\modManagerController) {
    ...
    {
        $input = $this->inputs[$field->get('input')];
        $icon_type = $field->get('icon_type');
        $icon_base_url = ($icon_type === 'core' || $icon_type === '') ? $this->coreIconUrl : $this->customIconUrl;

        $tmpField = $field->get(array('id', 'input', 'parent', 'parent_properties', 'name', 'description', 'content_desc', 'sortorder', 'icon', 'icon_type', 'properties', 'availability', 'layouts', 'settings', 'times_per_layout', 'times_per_page', 'process_tags', 'category'));
        $tmpField['icon'] =  $icon_base_url . $field->get('icon') . '--DPR--.png';
        $properties = (array)$this->modx->fromJSON($tmpField['properties'] ?? "[]");
        $tmpField['properties'] = $input ? $input->parsePathProperties($properties) : $properties;
    ...
    public function getAssets()
    {
        ...
        if (!$this->debug) {
            ...
            $js[] = $assetsUrl . 'js/vendor/autosize.min.js'.$version;
    ...
    {
        $eventOutput = $this->modx->event ? $this->modx->event->_output : '';
        $this->loadParser();
        ...
        try {
            ...
            foreach ($vcContent as $layout) {
                ...
                if (isset($layout['settings']) && is_array($layout['settings'])) {
                    foreach ($layout['settings'] as $id => $value) {
                        if (is_array($value) && is_array($settings) && !empty($settings)) {
                            $value = $this->processArraySetting($id, $value, $settings);
                        }
                        $columns[$id] = $value;
                ...
                foreach ($layout['content'] as $column => $fields) {
                    ...
                    foreach ($fields as $fieldData) {
                        ...
                        $fieldData['layout_title'] = $layoutTitle;
            ...
            $this->restoreParser();
            if ($this->modx->event) {
                $this->modx->event->_output = $eventOutput;
            }
            return $content;
        } catch (Exception $e) {
    ...
    /**
     * Process a setting that has an array value.
     * If the setting has a template field (like multi_select) it will parse the value into the template and joing them,
     * otherwise just the values are joined.
     *
     * @param string $settingRef
     * @param array $settingValue
     * @param array $settings This is a list of setting definitions to reference
     * @return string
     */
    public function processArraySetting(string $settingRef, array $settingValue, array $settings): string
    {
        if (!$settings) {
            return '';
        }
        ...
        foreach ($settings as $setting) {
            $outputArray = [];
            if ($setting['reference'] === $settingRef) {
                foreach ($settingValue as $value) {
                    if (isset($setting['template']) && !empty($setting['template'])) {
                        $outputArray[] = $this->parse($setting['template'], ['value' => $value]);
                    }
                    else {
                        $outputArray[] = $value;
                    }
            ...
            $separator = '';
            if (isset($setting['separator']) && !empty($setting['separator'])) {
                $separator = $setting['separator'];
            }
            $output .= implode($separator, $outputArray);
        }
        return $output;
    }
    ...
    public function generateFieldHtml($fieldData, $field = false, $idx = 0) {
        ...
        if ($field) {
            ...
            // Check for array field settings that need to be processed.
            if (!empty($fieldData['settings']) && is_array($fieldData['settings'])) {
                foreach ($fieldData['settings'] as $id => $value) {
                    if (is_array($value)) {
                        $fieldData['settings'][$id] = $this->processArraySetting($id, $value, $defSettings);
                    }
                }
            }
    ...
    public function parse ($tpl, $phs) {
        ...
        //
        // If the $tpl is a @CHUNK or @FILE, this also makes it so the chunk name or file name supports basic tags
        ...
        if (strpos(trim($tpl), '@CHUNK ') === 0) {
            $chunkName = substr($tpl, strlen('@CHUNK '));
            $chunk = '[[$' . $chunkName . '?';
            foreach ($phs as $key => $value) {
                if (is_scalar($value)) {
                    $chunk .= "\n &{$key}=`{$value}`";
        ...
        // Support for files by loading the file from a specific path and reading its contents
        if (strpos(trim($tpl), '@FILE ') === 0) {
            $source = $this->getFileTemplateSource();
            if (!$source) {
                $this->modx->log(1, '[ContentBlocks] Error: Media Source for file templates is not configured, but "' . $tpl . '" template definition found for parsing. Make sure the media source is available and the user has the appropriate permissions to use it.');
                return '<p class="error">Failed processing template, please refer to the error log.</p>';
            }

            // Grab the file from the @FILE tag and sanitize it
            $fileName = substr($tpl, strlen('@FILE '));
            $fileName = $this->sanitizePath($fileName);

            // Make sure it's always enclosed in slashes at the end only, and sanitize
            $basePath = trim($this->getOption('contentblocks.file_template_path', null, 'contentblocks/', true), '/') . '/';
            $basePath = $this->sanitizePath($basePath);

            // Combine the file path and sanitize it one last time
            $file = $this->sanitizePath($basePath . $fileName);

            // Ask the media source for the contents
            $contents = $source->getObjectContents($file);
            if (!is_array($contents) || !array_key_exists('content', $contents) || empty($contents['content'])) {
                $this->modx->log(1, "[ContentBlocks] Tried loading file \"{$file}\" for template definition \"{$tpl}\", but it could not be found or is empty. Please check your configuration.");
                return '<p class="error">Failed processing template, please refer to the error log.</p>';
            }

            // Set the template contents to the contents of the file, so the same parser will handle it as a regular tpl
            $tpl = $contents['content'];

            // Run basic replacement again, on the content instead of @FILE this time
            foreach ($phs as $key => $value) {
                if (is_scalar($value)) {
                    $tpl = str_replace('[[+' . $key . ']]', $value, $tpl);
                }
            ...
            // v  Deliberate pass-through to regular parsing v
        }
    ...
    public function cb()
    {
        ...
        $xml = $response instanceof modRestResponse
            ? $response->toXml()
        ...
        if ($valid) {
            ...
            if ($updatePublicKey) {
                file_put_contents($pubKeyFile, $this->modx->getOption('http_host') . '@@' . (string)$xml->pubkey);
            }
...
HTML;
    ...
    public function getAssetsForInputs() {
        ...
        foreach ($this->getFields() as $field) {
            /** @var cbField $field */
            $requestedInputs[] = $field->get('input');
            $dependencies = $field->getDependantInputs();
            if (!empty($dependencies) && is_array($dependencies)) {
                $requestedInputs = array_merge($requestedInputs, $dependencies);
            }
        }
        $requestedInputs = array_unique($requestedInputs);
        $js = array();
        $css = array();
        $tpl = array();
        ...
        if ($this->debug) {
            ...
            $js[] = $this->config['assetsUrl'] . 'js/inputs/checkbox.js';
            $js[] = $this->config['assetsUrl'] . 'js/inputs/multi-select.js';
        }
        else {
        ...
        foreach ($this->inputs as $key => $input) {
            if ($input instanceof cbInput) {
                $tjs = $input->getJavaScripts();
                if (!empty($tjs)) {
                    $js = array_merge($js, $tjs);
                }

                $tpls = $input->getTemplates();
                if (!empty($tpls)) {
                    $tpl = array_merge($tpl, $tpls);
                }

                $tcss = $input->getCss();
                if (!empty($tcss)) {
                    $css = array_merge($css, $tcss);
                }
                if (($index = array_search($key, $requestedInputs, true)) !== false) {
                    unset($requestedInputs[$index]);
                }
            }
            else {
                $this->modx->log(modX::LOG_LEVEL_ERROR, '[ContentBlocks] Input ' . $key . ' is not a valid cbInput.');
            }
        }

        foreach ($requestedInputs as $key) {
            $this->modx->log(modX::LOG_LEVEL_ERROR, '[ContentBlocks] Input "' . $key . '" was requested but does not exist.');
...
' . $content
    ...
    {
        if ($this->modx->parser && !($this->modx->parser instanceof \cbParser)) {
    ...
    {
        if ($this->normalParser instanceof modParser || $this->normalParser instanceof \MODX\Revolution\modParser) {
    ...
    public function getImagine()
    {
        ...
        if ($imagine) {
            if (ExifMetadataReader::isSupported()) {
                ...
                $this->modx->log(modX::LOG_LEVEL_ERROR, '[ContentBlocks] Can\'t use the ExifMetaDataReader to automatically rotate images: ' . ExifMetadataReader::getUnsupportedReason());
            }
    ...
    private function getFileTemplateSource(): ?modMediaSource
    {
        if (!$this->fileTemplateSource) {
            $this->modx->loadClass('sources.modMediaSource');
            $id = $this->getOption('contentblocks.file_template_source', null, 1, true);
            $source = modMediaSource::getDefaultSource($this->modx, $id);
            if ($source && $source->getWorkingContext() && $source->initialize()) {
                $this->fileTemplateSource = $source;
            }
        }

        if ($this->fileTemplateSource) {
            return $this->fileTemplateSource;
        }
    ...
    /**
     * Load the pdoTools service class if it exists, using the service loader for the current MODX version.
     */
    private function pdoTools()
    {
        if ($this->pdoTools) {
        ...
        if (isset($version['full_version']) && version_compare($version['full_version'], '3.0.0', '>=')){
            $this->pdoTools = $this->modx->services->has('pdotools') ? $this->modx->services->get('pdotools') : null;
        } else {
            $this->pdoTools = $this->modx->getService('pdoTools');
        }

        if (!$this->pdoTools) {
            $this->modx->log(modX::LOG_LEVEL_ERROR, '[ContentBlocks] pdoTools could not be loaded. Please make sure pdoTools is installed and up to date before using @PDO_FILE tags.');
        }
