...
(function ($) {
    ...
    window.ContentBlocks = {
        ...
        buildLayoutSwitcherContent: function(layoutId, fields, settings, title, unassignedSettings) {
            ...
            if (!meta) {
                container.append('<li class="contentblocks-layout">' +
                    ...
                    '<p class="error"><b>Layout #' + layoutId + ' was requested, but it does not exist.</b> The default layout #' + ContentBlocksConfig.default_layout + ' was also not found. This probably means you either have no layouts defined yet in the ContentBlocks component, the contentblocks.default_layout setting is not defined properly, or a layout that was in use has been removed. <a href="https://support.modmore.com/article/82-layout-requested-but-does-not-exist-error">Read more about this issue and how to resolve it.</a></p>' +
                    '<p>When you save this resource, the content of this layout will be deleted. If you may need to restore the content, copy the following raw data to a safe place before saving.</p><textarea class="contentblocks-no-default-layout">' + Ext.encode(content) + '</textarea>' +
                    '</div></div></li>');
                return false;
            }
            ...
            layout.find('.contentblocks-content').sortable({
                ...
                stop: function(event, ui) {
                    if (window.tinymce) {
                        ...
                        var tinymcerte2Instance = ui.item.find('.tox-tinymce');
                        if (tinymcerte2Instance.length > 0) {
                            var tinymcerte2Id = tinymcerte2Instance.siblings('textarea').attr('id');
                            tinymce.execCommand('mceRemoveEditor', true, tinymcerte2Id);
                            tinymce.execCommand('mceAddEditor', true, tinymcerte2Id);
                        }
                    }
                    if (window.CKEDITOR) {
        ...
        addExposedLayoutSettings: function(layout) {
            ...
            $(layout).find('.contentblocks-exposed-fields-wrapper .contentblocks-setting-color_picker').each(function() {
                var el = $(this);
                var id =  $(this).attr('id')
                settings.forEach(function(setting) {
                    if (setting.fieldtype === 'color_picker') {
                        ContentBlocks.initializeColorPickerSetting(el, {'id': id, settingType: 'layout'});
                    }
                });
            });

            $(layout).find('.contentblocks-exposed-fields-wrapper .contentblocks-setting-select').each(function() {
                var el = $(this);
                var id =  $(this).attr('id');
                settings.forEach(function(setting) {
                    if (id === setting.settingInstance + '-setting-' + setting.reference) {
                        ContentBlocks.initializeSelectSetting(el, {id: id, setting: setting, settingType: 'layout'});
                    }
                });

            });

            $(layout).find('.contentblocks-exposed-fields-wrapper .contentblocks-setting-multi_select').each(function() {
                var el = $(this);
                var id =  $(this).attr('id')

                settings.forEach(function(setting) {
                    if (id === setting.settingInstance + '-setting-' + setting.reference) {
                        ContentBlocks.initializeMultiSelectSetting(el, {id: id, setting: setting, settingType: 'layout'});
                    }
                });
            });
        ...
        generateSettingFields: function(settings, defaultSettings, currentData, fieldDisplayType, settingInstance) {
            ...
            var fields = [],
                fieldHasOptions = ['select', 'multi_select', 'radio', 'checkbox'];
            $.each(settings, function(id, setting) {
        ...
        /**
         * Layout setting as modal
         */
        openLayoutSettings: function(param, layoutId) {
            var l;
            if (typeof layoutId !== 'undefined') {
                l = $(layoutId);
            }
            else {
                l = $(this).closest('li.contentblocks-layout');
                localStorage.setItem('layout-settings-modal-id', l.attr('id'));
            }

            var lId = l.data('layout'),
            ...
            ContentBlocks.openModal(_('contentblocks.layout_settings.modal_header', {name: layoutMeta.name}), html, {
                ...
                initCallback: function(modal) {
                    ...
                    modal.find('.contentblocks-setting-color_picker').each(function() {
                        var el = $(this);
                        var id =  $(this).attr('id')
                        settings.forEach(function(setting) {
                            if (id === setting.settingInstance + '-setting-' + setting.reference) {
                                ContentBlocks.initializeColorPickerSetting(el, {'id': id, settingType: 'modal'});
                            }
                        });
                    });

                    modal.find('.contentblocks-setting-select').each(function() {
                        var el = $(this);
                        var id =  $(this).attr('id')
                        settings.forEach(function(setting) {
                            if (id === setting.settingInstance + '-setting-' + setting.reference) {
                                ContentBlocks.initializeSelectSetting(el, {id: id, setting: setting, settingType: 'modal'});
                            }
                        });
                    });

                    modal.find('.contentblocks-setting-multi_select').each(function() {
                        var el = $(this);
                        var id =  $(this).attr('id')
                        settings.forEach(function(setting) {
                            if (id === setting.settingInstance + '-setting-' + setting.reference) {
                                ContentBlocks.initializeMultiSelectSetting(el, {id: id, setting: setting, settingType: 'modal'});
                            }
                        });
                    });
        ...
        addExposedFieldSettings: function(fld) {
            ...
            fld.find('.contentblocks-setting-color_picker').each(function() {
                var el = $(this);
                var id =  $(this).attr('id')
                settings.forEach(function(setting) {
                    if (id === setting.settingInstance + '-setting-' + setting.reference) {
                        ContentBlocks.initializeColorPickerSetting(el, {'id': id, settingType: 'field'});
                    }
                });
            });

            fld.find('.contentblocks-setting-select').each(function() {
                var el = $(this);
                var id =  $(this).attr('id')
                settings.forEach(function(setting) {
                    if (id === setting.settingInstance + '-setting-' + setting.reference) {
                        ContentBlocks.initializeSelectSetting(el, {id: id, setting: setting, settingType: 'field'});
                    }
                });
            });

            fld.find('.contentblocks-setting-multi_select').each(function() {
                var el = $(this);
                var id =  $(this).attr('id')
                settings.forEach(function(setting) {
                    if (id === setting.settingInstance + '-setting-' + setting.reference) {
                        ContentBlocks.initializeMultiSelectSetting(el, {id: id, setting: setting, settingType: 'field'});
                    }
                });
            });
        ...
        addLayoutModal: function(event) {
            ...
            if (btn.hasClass('contentblocks-add-layout-here') || btn.hasClass('contentblocks-switch-layout')) {
        ...
        utilities: {
            ...
            getThumbnailUrl: function(url, size, source) {
                ...
                if (size > 0 || size.length > 0) {
                    ...
                    // Only return a thumbnail if the width and height are larger than 0
                    if (width > 0 && height > 0) {
                        thumbUrl += '?src=/' + normalised.cleanedSrc;
                        thumbUrl += '&w=' + width + '&h=' + height + '&zc=1';
                        if (normalised.cleanedSrc.indexOf('png') > 0) {
                            thumbUrl += '&f=png';
                        }
                        if (source) {
                            thumbUrl += '&source=' + source;
                        }
        ...
        initializeColorPickerSetting: function(el, data) {
            var defaults = MODx.config['contentblocks.color_picker_defaults'].split(',');
            var swatch = [];
            for (var i = 0; i < 16; i++) {
                swatch['swatch' + (i + 1)] = defaults[i];
            }
            var id = $(el).attr('id'),
                dom = $('#' + id),
                input = dom.find('input[type=hidden]'),
                reference = dom.find('button').data('reference'),
                value = input.val(),
                inputName = input.attr('name'),
                fldInstance = ContentBlocks.fieldTypes.color_picker(dom, {
        ...
        initializeSelectSetting: function(el, data) {
            var id = $(el).attr('id'),
                dom = $('#' + id),
                input = dom.find('select'),
                reference = data.setting.reference,
                setting = data.setting,
                value = data.setting.value,
                fldInstance = ContentBlocks.fieldTypes.dropdown(dom, {
                    settingType: data.settingType,
                    reference: reference,
                    value: value,
                    input: input,
        ...
        initializeMultiSelectSetting: function(el, data) {
            var id = $(el).attr('id'),
                dom = $('#' + id),
                input = dom.find('select'),
                reference = data.setting.reference,
                setting = data.setting,
                value = data.setting.value,
                fldInstance = ContentBlocks.fieldTypes.multi_select(dom, {
                    settingType: data.settingType,
                    reference: reference,
                    value: value,
                    input: input,
        ...
        initializeLinkField : function(input, data) {
            ...
            // Wrap the link input and displayTextHolder in a container
            var linkContainer = $('<div class="contentblocks-field-link-container"></div>');
            $link.before(linkContainer);
        ...
        render: function(contentField) {
            var selector = '#modx-resource-content > .x-panel-bwrap, .contentblocks_replacement';

            // MODX 3 has a different DOM structure. Plus we have the extra step of hiding the content label element.
            if (ContentBlocksWrapperCls.indexOf('modx_v30') > -1) {
                selector = '#modx-resource-content > .x-panel-bwrap > .x-panel-body > .x-form-item > .x-form-element, .contentblocks_replacement';
                $('#modx-resource-content > .x-panel-bwrap > .x-panel-body > .x-form-item > .x-form-item-label').hide();
            }
            
            contentField = contentField || selector;
            ...
            // Initialize into the right container (ignore this if using the template builder)
            if (!window.isContentBlocksTemplateBuilder) {
                if (pos === 'block') {
                    ...
                    resourcePanel.add({
                        ...
                        collapsible: true,
                        animCollapse: true,
                        // Only MODX 3 should have a margin
                        style: MODx.config.version.charAt(0) > '2' ? 'margin: 0 16px 0;' : ''
                    });
