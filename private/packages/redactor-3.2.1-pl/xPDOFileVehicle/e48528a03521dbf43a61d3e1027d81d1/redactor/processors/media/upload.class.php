    ...
    public function process()
    {
        ...
        foreach ($_FILES as $upload) {
            ...
            if (!empty($newFileName)) {
                $baseMediaPath = $this->source->getBasePath();
                ...
                $baseMediaUrl = $this->source->getBaseUrl();
                $baseMediaUrl = rtrim($baseMediaUrl, '/') . '/';
                if (strpos($newFileName, $baseMediaUrl) === 0) {
                    $newFileName = substr($newFileName, strlen($baseMediaUrl));
                }

                if (strpos($newFileName, $path) === false) {
                    $newFileName = ltrim($path, '/') . '/' . $newFileName;
                }

                $newLink = $this->source->getObjectUrl($newFileName);
                $file['url'] = $newLink;
            }
            $results['file-' . $i] = $file;
        }
