...
final class Redactor
{
    ...
    /**
     * @var modX|\MODX\Revolution\modX A reference to the modX object.
     */
    public $modx = null;

    /**
     * @var modContext|\MODX\Revolution\modContext A reference to the current working context object.
     */
    private $wctx = null;
    ...
    public function __construct($modx,array $config = array()) {
        $this->modx = $modx;
        $this->version = new VersionObject(3, 2, 1, 'pl');
    ...
    */
    ...
    public function getGeneratedConfigurationSets(array $options = []): string
    {
        ...
        if (array_key_exists(self::OPT_IS_FRED, $options) && $options[self::OPT_IS_FRED]) {
            $urlParams[self::OPT_IS_FRED] = 1;
        }
        if ($ctx = $this->getWorkingContext()) {
            $urlParams['wctx'] = $ctx->get('key');
        }
        if ($resource = $this->getWorkingResource()) {
            $urlParams['resource'] = $resource->get('id');
        }
        $outputUrl .= '&' . http_build_query($urlParams);

        $output[] = '<script src="' . $outputUrl . '" type="text/javascript"></script>';

        $major = (int)$this->modx->getVersionData()['version'];
        $output[] = <<<HTML
<script type="text/javascript">document.documentElement.className += ' redactor-modx{$major}';</script>
HTML;
...
HTML;
    ...
    {
        if ($key instanceof modResource || $key instanceof \MODX\Revolution\modResource) {
    ...
    public function r(): void
    {
        ...
        /** @var modRestResponse|\Psr\Http\Message\ResponseInterface $response */
