...
//                'leaveSubmitMethodAlone' => true,
        ...
        // Add a MODX-optimised fixed toolbar - enabled by default but can be turned off
        if (!array_key_exists('toolbarFixed', $options) || $options['toolbarFixed']) {
        ...
        // Control if data uris should be accepted in images
        // This is done as a system setting as it's a security measure to block it (the default) and shouldn't
        // be toggled on a per-config-set-basis.
        $options['imageSrcData'] = (bool)$this->redactor->getOption('redactor.allow_data_uris', null, false);
