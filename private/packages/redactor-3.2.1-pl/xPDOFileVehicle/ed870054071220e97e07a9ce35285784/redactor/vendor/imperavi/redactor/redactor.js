...
$R.add('service', 'cleaner', {
    ...
    input: function(html, paragraphize, started)
    {
        ...
        $wrapper.find('a, iframe, embed').each(function(node) {
            ...
            /**
             * MODMORE PATCH FOR #540, manager url getting prefixed
             */
            let managerUrlPos = href ? href.indexOf(MODx.config.manager_url) : -1,
                nextCharPos = managerUrlPos >= 0 ? managerUrlPos + MODx.config.manager_url.length : -1,
                next2Chars = nextCharPos >= 0 ? href.substring(nextCharPos, nextCharPos + 2) : '';

            if (next2Chars === '[[') {
                console.debug('[Redactor] Fixing href ' + href + ' with ' + href.substring(nextCharPos));
                href = href.substring(nextCharPos);
                $node.attr('href', href);
            }
            /**
             * END MODMORE PATCH FOR #540, manager url getting prefixed
             */
        });
...
$R.add('class', 'link.component', {
    ...
    _set_url: function(url)
    {
        if (this.opts.linkValidation)
        {
            ...
            {
                /**
                 * MODMORE PATCH #541 http -> https
                 */
                url = 'https://' + url.replace(/(ftp|https?):\/\//i, '');
                /**
                 * END MODMORE PATCH
                 */
            }
