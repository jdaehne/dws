    ...
    $R.add('plugin', 'better_imagemanager', {
        ...
        browserCallback: function(imageData) {
            ...
            var url = imageData.fullRelativeUrl,
                key = 'image_' + ++this._insertedIdx;

            this._insertedImages[key] = {
                id: key,
                url: url,
            };

            if (this._insertDelay) {
                clearTimeout(this._insertDelay);
            }

            this._insertDelay = setTimeout(() => {
                this.app.api('module.image.insert', this._insertedImages);
                this._insertedImages = {};
            }, 25);
