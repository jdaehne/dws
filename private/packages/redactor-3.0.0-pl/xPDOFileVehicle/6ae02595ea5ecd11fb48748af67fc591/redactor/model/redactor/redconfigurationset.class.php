...
class redConfigurationSet extends xPDOSimpleObject
{
    ...
    protected function processOptions(array $options): array
    {
        ...
        $simpleButtonPlugins = ['alignment', 'fontcolor', 'fontfamily', 'fontsize', 'fullscreen', 'properties',
            'specialchars', 'table', 'textdirection', 'video', 'widget', 'divider', 'download',
        ];
        ...
        if (in_array('inline', $buttons, true) || in_array('inlinestyle', $buttons, true)) {
...
//                'leaveSubmitMethodAlone' => true,
        ...
        // Set the linkResourceUrl
        $ctx = $this->redactor->getWorkingContext();
        $contextParams = '&' . http_build_query([
            'context' => $ctx ? $ctx->get('key') : 'web'
        ]);
        $options['linkResourceUrl'] = $this->redactor->config['assetsUrl'] . 'connector.php?action=resources/search&'.$urlParams.$contextParams;
