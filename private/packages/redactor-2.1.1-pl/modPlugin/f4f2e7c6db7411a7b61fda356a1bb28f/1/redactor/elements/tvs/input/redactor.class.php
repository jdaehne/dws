...
class RedactorInputRender extends modTemplateVarInputRender {
    ...
    public function render($value, array $params = array()) {
        ...
        if($params['eureka'] == '1' && !(bool)$this->redactor->getOption('redactor.plugin_eureka', null, true)) {
            ...
            $this->modx->controller->addCSS($this->redactor->assetsUrl . 'lib/eureka/css/eureka.1.1.0.min.css');
            ...
            if((bool)$this->redactor->getOption('redactor.plugin_eureka_shivie9', null, true)) {
                $this->modx->controller->addJavascript($this->redactor->assetsUrl . 'lib/eureka/js/eureka.dom4.1.1.0.min.js');
                $this->modx->controller->addJavascript($this->redactor->assetsUrl . 'lib/eureka/js/eureka.no-flexbox.1.1.0.min.js');
            }
            $this->modx->controller->addJavascript($this->redactor->assetsUrl . 'lib/eureka/js/muckboot.eureka.1.1.0.min.js');
            $this->modx->controller->addJavascript($this->redactor->assetsUrl . 'lib/eureka/js/eureka.1.1.0.min.js');
        }
...
HERE;
    ...
    protected function registerStuff() {
        $this->modx->controller->addCSS($this->redactor->config['assetsUrl'].'redactor-2.1.1.min.css');
        ...
        $this->modx->controller->addJavascript($this->redactor->config['assetsUrl'].'redactor-2.1.1.min.js');
$script = <<<HERE
<script>try {ace} catch(e) { document.write('<script src="https://cdnjs.cloudflare.com/ajax/libs/ace/1.1.9/ace.js"><\/script>') }</script>
HERE;
